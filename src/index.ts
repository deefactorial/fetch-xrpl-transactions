...
async function initalizeClient() {
    ...
    const clientWithAuth = Client.withUserAuth(userAuth)

    return clientWithAuth;
}

const XRPLNodeUrl = typeof process.env.WEB_SOCKET === 'undefined' ? 'wss://s2.ripple.com' : process.env.NODE.trim()
...
console.log('Websocket', XRPLNodeUrl)
    ...
    const fetchLedgerTransactions = (ledger_index) => {
        return new Promise((resolve, reject) => {
            return Connection.send({
                ...
                const transaction = await client.writeTransaction(ThreadID.fromString('bafkxwsshckpo5unnw6xzwfevrgvk4vaoz3wm3dshi4ueghe57rtkuuq'), 'ledgers');
